import { useState, useEffect } from "react";
import { FaClosedCaptioning, FaMicrophone, FaFileAlt } from "react-icons/fa";

const ServerSelector = ({ episodeId, setSelectedServer, setCategory }) => {
  const [servers, setServers] = useState({ sub: [], dub: [], raw: [] });
  const [loading, setLoading] = useState(true);
  const [activeServer, setActiveServer] = useState(null);
  const [activeCategory, setActiveCategory] = useState(null);

  const pickDefaultServer = (serverGroups) => {
    const preferredSub = (serverGroups.sub || []).find((item) => item.serverName === 'hd-2');
    if (preferredSub) return { category: 'sub', serverName: preferredSub.serverName };

    const firstSub = (serverGroups.sub || [])[0];
    if (firstSub) return { category: 'sub', serverName: firstSub.serverName };

    const firstDub = (serverGroups.dub || [])[0];
    if (firstDub) return { category: 'dub', serverName: firstDub.serverName };

    const firstRaw = (serverGroups.raw || [])[0];
    if (firstRaw) return { category: 'raw', serverName: firstRaw.serverName };

    return null;
  };

  useEffect(() => {
    const fetchServers = async () => {
      if (!episodeId) return;

      setLoading(true);
      try {
        const res = await fetch(`/api/server?episodeId=${encodeURIComponent(episodeId)}`);
        if (!res.ok) throw new Error("Failed to fetch episode servers");

        const data = await res.json();
        const nextServers = {
          sub: data.sub || [],
          dub: data.dub || [],
          raw: data.raw || [],
        };

        setServers(nextServers);

        const initial = pickDefaultServer(nextServers);
        if (initial) {
          setSelectedServer(initial.serverName);
          setCategory(initial.category);
          setActiveServer(initial.serverName);
          setActiveCategory(initial.category);
        }
      } catch (error) {
        console.error("Error fetching servers:", error);
        setServers({ sub: [], dub: [], raw: [] });
      } finally {
        setLoading(false);
      }
    };

    fetchServers();
  }, [episodeId]);

  const handleSelect = (serverName, category) => {
    setSelectedServer(serverName);
    setActiveServer(serverName);
    setActiveCategory(category);
    setCategory(category);
  };

  const renderServers = (category, serversList, icon) => {
    if (loading) {
      return (
        <div className="mt-4">
          <div className="flex items-center gap-2 text-lg font-semibold">
            {icon} <span className="capitalize">{category}</span>
          </div>
          <div className="flex flex-wrap gap-2 mt-2 border-b border-gray-700 pb-2">
            {Array.from({ length: 3 }).map((_, idx) => (
              <div
                key={idx}
                className="w-20 h-8 bg-gray-700 rounded animate-pulse"
              ></div>
            ))}
          </div>
        </div>
      );
    }

    if (!serversList.length) return null;

    return (
      <div className="mt-4">
        <div className="flex items-center gap-2 text-lg font-semibold">
          {icon} <span className="capitalize">{category}</span>
        </div>
        <div className="flex flex-wrap gap-2 mt-2 border-b border-gray-700 pb-2">
          {serversList.map((server, index) => (
            <button
              key={`${category}-${server.serverId || server.serverName}-${index}`}
              onClick={() => handleSelect(server.serverName, category)}
              className={`px-4 py-2 rounded transition ${
                activeServer === server.serverName && activeCategory === category
                  ? "bg-blue-600 text-white"
                  : "bg-gray-800 hover:bg-gray-600"
              }`}
            >
              {server.serverName}
            </button>
          ))}
        </div>
      </div>
    );
  };

  try {
    return (
      <div className="mt-4">
        {renderServers(
          "sub",
          servers.sub,
          <FaClosedCaptioning className="text-yellow-400" />
        )}
        {renderServers(
          "dub",
          servers.dub,
          <FaMicrophone className="text-red-400" />
        )}
        {renderServers(
          "raw",
          servers.raw,
          <FaFileAlt className="text-green-400" />
        )}
      </div>
    );
  } catch (error) {
    console.error("Render error in ServerSelector:", error);
    return (
      <div className="text-red-500 mt-4">
        Oops, something went wrong while rendering servers.
      </div>
    );
  }
};

export default ServerSelector;