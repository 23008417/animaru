...
function SearchModal({ onClose }) {
  ...
  useEffect(() => {
    if (query.length > 2) {
      setLoading(true);
      fetch(`/api/consumet/meta/anilist/${encodeURIComponent(query)}`)
        .then((res) => res.json())
        .then((data) => {
...
function AvatarDropdown() {
  ...
  const handleAvatarClick = () => {
    if (!loading && user) setShowDropdown((prev) => !prev);
  };
    ...
    >
      <Navbar className="bg-transparent">
        <NavbarBrand>
          <Link
            href="/"
            className="text-white font-semibold text-2xl tracking-wide uppercase no-underline hover:no-underline"
