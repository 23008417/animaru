...
const EpisodeGuide = ({ animeId, epnum, progress }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/provider/aniwatch/episodes?animeId=${animeId}`);
        ...
        if (res.ok && Array.isArray(data?.episodes)) {
          ...
          throw new Error(data?.error || "Failed to fetch episodes from provider.");
        }
      } catch (err) {
          ...
          {episodeData.map((episode, index) => {
            ...
            return (
              <Link
                ...
                href={`/watch/${encodeURIComponent(episode.episodeId)}?anilist=${animeId}`}
