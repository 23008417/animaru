...
const resolveGenreFromRoute = (routeId) => {
  if (!routeId) return null;

  const decoded = decodeURIComponent(String(routeId));

  const exactMatch = allGenres.find(
    (genre) => genre.toLowerCase() === decoded.toLowerCase()
  );
  if (exactMatch) return exactMatch;

  const normalizedRoute = normalizeGenre(decoded);
  const normalizedMatch = allGenres.find(
    (genre) => normalizeGenre(genre) === normalizedRoute
  );
...
const GenrePage = () => {
  ...
  const { id } = useParams();
  const genreFromRoute = resolveGenreFromRoute(id);

  useEffect(() => {
    if (genreFromRoute || selectedGenres.length > 0 || year || season || country) {
      setLoading(true);
  ...
  }, [genreFromRoute, selectedGenres, page, year, season, episodeRange, country]);
